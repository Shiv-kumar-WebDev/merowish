						...
						<table class="display" id="responsive">
							...
							<tbody>
								...
								<tr>
									<td>{{$row->code}}</td>
									<td>{{$row->type}} </td>
									<td>{{ $row->type=='Percentage' ? $row->value.' %' : $row->value.' ₹' }} </td>
									{{-- <td>{{$row->category}}</td> --}}
									{{-- <td>{{ $row->content }}</td> --}}
									<?php
									...
									<td
										...
										style="width: 50px;">
										<a href="{{ route('CouponCodeEdit', ['id' => $row->id]) }}" style="color:Black;"><i data-feather="edit-2"></i></a>
										<a href="{{ route('CouponCodeDelete', ['id' => $row->id]) }}" style="color:red;"><i data-feather="trash-2"></i></a>
									</td>
								</tr>
								@endforeach
							</tbody>
