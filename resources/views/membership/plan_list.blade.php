				...
				<div class="card-body">
					<div class="dt-ext table-responsive">
						<table class="display" id="responsive">
							...
							<tbody>
								@foreach ($plans_data as $plans)
								<tr>
									<td>{{$plans->name}}</td>
									<td>{{$plans->features}}</td>
									<td>
										@if($plans->duration == 1)
											Yearly
										@elseif($plans->duration == 2)
											Monthly
										@elseif($plans->duration == 3)
											Half Yearly
										@elseif($plans->duration == 4)
											Quaterly
										@endif
									</td>
									<td>{{$plans->price}}</td>
									<td>{{$plans->allow_renew}}</td>
									<td>{{$plans->allow_extend}}</td>
									<td>{{$plans->active}}</td>
									<td
										class="jsgrid-cell jsgrid-control-field jsgrid-align-center"
										style="width: 50px;">
										<a href="{{ route('MembershipPlanEdit', ['id' => $plans->id_membership_plan]) }}" style="color:Black;"><i data-feather="edit-2"></i></a>
										<a href="{{ route('MembershipPlanDelete', ['id' => $plans->id_membership_plan]) }}" style="color:red;"><i data-feather="trash-2"></i></a>
									</td>
								</tr>
								@endforeach
							</tbody>
