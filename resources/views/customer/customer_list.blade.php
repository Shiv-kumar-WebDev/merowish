...
<h3>Customer Membership Plans</h3>
@endsection

@section('breadcrumb-items')
<li class="breadcrumb-item">Membership</li>
<li class="breadcrumb-item active">Customer Plans</li>
@endsection
					...
					<div class="dt-ext table-responsive">
						<table class="display" id="responsive">
							...
							<tbody>
								...
								<tr>
									<td>{{$row->Social_title}}</td>
									<td>{{$row->firstname}}</td>
									<td>{{$row->lastname}}</td>
									<td>{{$row->email}}</td>
									<td>{{$row->birthday}}</td>
									<td>{{$row->Group_access}}</td>
									<td
										class="jsgrid-cell jsgrid-control-field jsgrid-align-center"
										style="width: 50px;">
										<a href="{{ route('CustomerEdit', ['id' => $row->id_customer]) }}" style="color:Black;"><i data-feather="edit-2"></i></a>
										<a href="{{ route('CustomerDelete', ['id' => $row->id_customer]) }}" style="color:red;"><i data-feather="trash-2"></i></a>
									</td>
								</tr>
								@endforeach
							</tbody>
